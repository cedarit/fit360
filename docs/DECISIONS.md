...
| 10 | First vertical lesson slice topic | (a) Module 0, Chapter 0.1 "What Fit360 is and isn't" — no anatomy/physiology claims to source, fastest to produce, sets expectations before anything else; **recommended**, and drafted in full at [lessons/term-01-module-0-chapter-1-lesson-1.md](curriculum/lessons/term-01-module-0-chapter-1-lesson-1.md) as the example. (b) Module 1, Chapter 1.1 "Anatomical position, body regions and organ systems" — more visually engaging and closer to the Lat Pulldown Lab's prerequisites, but needs sourced anatomical content sign-off first. Not decided; FIT-5 writes (a) as the example without treating the choice as settled. | Before FIT-5 sign-off |
| 11 | Citing/quoting CC BY-NC-SA academic sources (e.g. OpenStax) once Fit360 is a paid product | Lesson 1.1.1 (FIT-10) cites OpenStax *Anatomy and Physiology 2e*, which is CC BY-NC-SA (NonCommercial), not CC BY 4.0 as an earlier draft wrongly stated. Citing facts in original prose (not reproducing the source's text, tables or images) is standard practice and likely outside what the licence restricts, but Fit360's own rule is "original or licensed assets only" (AGENTS.md). Options: (a) keep citing/paraphrasing facts from NC-licensed academic sources in original wording indefinitely, treating that as distinct from "reusing" the licensed work; (b) never use any NC-licensed source's actual diagrams/images/tables, even paraphrased, without separate commercial permission (already implied by AGENTS.md, but not yet stated for text specifically); (c) get explicit legal sign-off on (a) before the app is a paid product. This number will likely need renumbering once FIT-6's own new Open item (also added as #11, on a separate branch not yet merged) lands — flagged in the FIT-10 PR. | Before any paid launch that reuses this kind of source |
